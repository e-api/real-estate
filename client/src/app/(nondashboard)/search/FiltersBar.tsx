...
const FiltersBar = () => {
  ...
  const handleLocationSearch = async () => {
    try {
      // Replace Mapbox API with Nominatim API
      const response = await fetch(
        `https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(searchInput)}&addressdetails=1&limit=1`
      );
      const data = await response.json();

      // Check if we have results
      if (data && data.length > 0) {
        const { lat, lon } = data[0]; // Nominatim returns lat and lon
        dispatch(
          setFilters({
            location: searchInput,
            coordinates: [parseFloat(lon), parseFloat(lat)], // Ensure the coordinates are numbers
          })
        );
      }
    } catch (err) {
