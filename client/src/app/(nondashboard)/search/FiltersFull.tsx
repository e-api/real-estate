...
const FiltersFull = () => {
  ...
  const handleLocationSearch = async () => {
    try {
      // Replace Mapbox URL with Nominatim URL
      const response = await fetch(
        `https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(localFilters.location)}&addressdetails=1&limit=1`
      );
      const data = await response.json();

      if (data && data.length > 0) {
        const [lng, lat] = [data[0].lon, data[0].lat];
        setLocalFilters((prev) => ({
          ...prev,
          coordinates: [parseFloat(lng), parseFloat(lat)], // Parse the coordinates to ensure they are numbers
        }));
      }
    } catch (err) {
